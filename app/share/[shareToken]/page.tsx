    ...
    <div className="min-h-screen bg-bg flex flex-col font-sans">
      ...
      <header className="h-16 border-b border-border bg-surface px-6 flex items-center justify-between shadow-sm sticky top-0 z-10">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-text-primary text-lg tracking-tight">oneclickresult</span>
        </div>
        <a 
          href={url!} 
          download={name}
        ...
        <div className="max-w-4xl w-full bg-surface border border-border rounded-xl shadow-lg overflow-hidden flex flex-col md:flex-row min-h-[500px]">
          ...
          <div className="p-6 border-b md:border-b-0 md:border-r border-border bg-bg-secondary/10 w-full md:w-80 flex flex-col shrink-0">
            <div className="flex items-center gap-4 mb-6">
               ...
               <div>
                  <h3 className="text-xs font-semibold text-text-tertiary uppercase tracking-wider mb-0.5">Shared File</h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs text-text-tertiary font-medium mb-1 block">Filename</label>
                <h1 className="font-semibold text-text-primary text-lg break-words leading-tight">{name}</h1>
              </div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-text-tertiary font-medium mb-1 block">Size</label>
                  <p className="text-sm text-text-secondary font-mono">{formattedSize}</p>
                </div>
                <div>
                   <label className="text-xs text-text-tertiary font-medium mb-1 block">Uploaded</label>
                   <p className="text-sm text-text-secondary">{new Date(_creationTime).toLocaleDateString()}</p>
                </div>
              </div>
               ...
               <div className="mt-6 pt-6 border-t border-border">
                  <a 
                    href={url!} 
                    download={name}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-text-primary text-bg-primary rounded-lg hover:bg-black/90 transition-all font-medium text-sm"
                  >
                    <Download className="w-4 h-4" />
                    Download File
                  </a>
